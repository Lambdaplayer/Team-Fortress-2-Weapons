...
ENT.Model = Model("models/weapons/w_models/w_drg_ball.mdl")
...
function ENT:FindTarget()
	...
	for _,v in pairs(ents.GetAll()) do
		if (v:IsPlayer() or v:IsNPC()) and v:Health()>0 and self:GetOwner() then
...
function ENT:DoExplosion(ent)
	...
	if self.Nuke then
		...
		--[[if self.FastRocket then
			...
			self:EmitSound( "weapons/cow_mangler_explosion_normal_0"..math.Round(math.Rand(1,6))..".wav") 
		--end
		...
		ParticleEffect("drg_cow_explosioncore_normal", self:GetPos(), self:GetAngles())
